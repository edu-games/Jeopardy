...
import { games, teams, students, gameState } from './schema';
...
export class GameHub implements DurableObject {
	...
	async fetch(request: Request): Promise<Response> {
		...
		if (url.pathname === '/ws') {
			...
			if (role === 'instructor' || role === 'projector') {
				...
				const connectedStudentIds = this.state
					.getWebSockets()
					.filter((ws) => this.state.getTags(ws)[0] === 'student')
					.map((ws) => this.state.getTags(ws)[3])
					.filter((id): id is string => id !== '');
				server.send(
					JSON.stringify({ type: 'connected-students-snapshot', studentIds: connectedStudentIds })
				);
		...
		try {
			switch (event.type) {
				...
				case 'close-question':
					...
					break;
				case 'submit-wager':
					await this.handleSubmitWager(
						db,
						ws,
						gameId,
						instructorId,
						event.teamId as string,
						event.wager as number
					);
					break;
				case 'answer':
					await this.handleAnswer(
						...
						gameId,
						instructorId,
						event.isCorrect as boolean,
						event.teamId as string
					);
					break;
				case 'assign-team':
					await this.handleAssignTeam(
						db,
						ws,
						gameId,
						instructorId,
						event.studentId as string,
						event.teamId as string
					);
					break;
				case 'enable-buzzer':
	...
	private async handleRevealQuestion(
		db: DB,
		ws: WebSocket,
		gameId: string,
		instructorId: string,
		...
		const buzzerEnabled = !slot.isWildCard;
	...
	private async handleAnswer(
		...
		if (!currentSlotId)
			return ws.send(JSON.stringify({ type: 'error', message: 'No active question' }));
		...
		if (slot.isWildCard) {
			if (!game.gameState?.currentWager) {
				return ws.send(
					JSON.stringify({ type: 'error', message: 'No wager submitted for Wild Card' })
				);
			}
	...
	private async handleSubmitWager(
		...
		if (!slot?.isWildCard) {
			return ws.send(
				JSON.stringify({ type: 'error', message: 'Current question is not a Wild Card' })
			);
		}
	...
	private async handleBuzzer(db: DB, ws: WebSocket, gameId: string, studentId: string) {
		if (!studentId)
			return ws.send(JSON.stringify({ type: 'error', message: 'Student ID required' }));
		...
		await db
			.update(students)
			.set({ buzzerAt, updatedAt: buzzerAt })
			.where(eq(students.id, studentId));

		await db
			.update(gameState)
			.set({ buzzerEnabled: false, updatedAt: buzzerAt })
			.where(eq(gameState.gameId, gameId));
	...
	private async handleEnableBuzzer(db: DB, ws: WebSocket, gameId: string, instructorId: string) {
		...
		await db
			.update(gameState)
			.set({ buzzerEnabled: true, updatedAt: now })
			.where(eq(gameState.gameId, gameId));
