...
export function addClient(clientId: string, gameId: string, controller: ReadableStreamDefaultController) {
	clients.set(clientId, { id: clientId, controller, gameId });
	console.log(`[SSE] Client ${clientId} connected to game ${gameId}. Total clients: ${clients.size}`);
}
...
export function getClientsByGame(gameId: string): SSEClient[] {
	return Array.from(clients.values()).filter((client) => client.gameId === gameId);
}

export type GameEvent =
	| { type: 'question-revealed'; slotId: string; currentSlot: any; buzzerEnabled: boolean }
	| { type: 'answer-submitted'; teams: any[]; answeredSlots: string[]; currentSlotId: string | null }
	| { type: 'buzzer-pressed'; studentId: string; studentName: string; teamId: string; teamName: string; buzzerAt: string }
...
export function broadcastToGame(gameId: string, event: GameEvent) {
	...
	console.log(`[SSE] Broadcasting ${event.type} to ${gameClients.length} clients in game ${gameId}`);
