<script lang="ts">
	...
	function parseCSV(csv: string): Array<{ clue: string; response: string; tags: string[] }> {
		...
		if (clueIndex === -1 || responseIndex === -1) {
			throw new Error('CSV must have "clue" and "response" columns');
		}
		...
		for (let i = 1; i < lines.length; i++) {
			...
			const clue = values[clueIndex]?.trim();
			const response = values[responseIndex]?.trim();
			const tagsString = tagsIndex !== -1 ? values[tagsIndex]?.trim() : '';

			if (!clue || !response) {
				throw new Error(`Row ${i + 1}: clue and response are required`);
			}
			...
			questions.push({ clue, response, tags });
		}
		...
		try {
			...
			if (importFormat === 'json') {
				...
				if (!data.questions || !Array.isArray(data.questions)) {
					throw new Error('Invalid format: must have a "questions" array');
				}
			...
			result = {
				success: false,
				message: err instanceof Error ? err.message : `Invalid ${importFormat.toUpperCase()} format`
			};
		} finally {
			importing = false;
		}
	...
	function loadSampleData() {
		if (importFormat === 'json') {
			inputData = JSON.stringify(
				{
					...
					questions: [
						{
							clue: 'This Founding Father is on the $100 bill',
						...
						{
							clue: 'The process by which plants make food',
			...
			inputData = `clue,response,tags
This Founding Father is on the $100 bill,Benjamin Franklin,History;American History
The capital of France,Paris,Geography;Europe
The process by which plants make food,Photosynthesis,Science;Biology`;
		}
	}
</script>
...
<div class="max-w-4xl">
	<div class="mb-8">
		<div class="flex items-center gap-2 mb-2">
			<a href="/dashboard/questions" class="text-gray-600 hover:text-gray-900 transition-colors">
	...
	<div class="bg-white rounded-lg shadow p-6 mb-6">
		<label class="block text-sm font-medium text-gray-700 mb-3">Import Format</label>
	...
	<div class="bg-blue-50 border border-blue-200 rounded-lg p-6 mb-6">
		...
		{#if importFormat === 'json'}
			<p class="text-blue-800 mb-3">Your JSON file should have the following structure:</p>
			<pre class="bg-white p-4 rounded border border-blue-200 text-sm overflow-x-auto"><code
					>{JSON.stringify(
						{
							version: '1.0',
							questions: [
								{
									clue: 'The clue shown to players',
				...
				></pre>
		{:else}
			<p class="text-blue-800 mb-3">Your CSV file should have the following structure:</p>
			<ul class="text-blue-800 mb-3 list-disc list-inside space-y-1">
				<li>
					First row must be headers: <code class="bg-white px-1 rounded">clue,response,tags</code>
				</li>
