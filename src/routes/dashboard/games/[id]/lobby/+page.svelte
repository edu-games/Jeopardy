<script lang="ts">
    ...
    function startGame() {
        if (
            ...
            if (!confirm(`There are ${unassignedStudents.length} unassigned students. Start game anyway?`)) {
                return;
            }
        }
        starting = true;
...
<div class="max-w-7xl mx-auto">
    ...
    <div class="flex items-center justify-between mb-8 gap-4">
        ...
        <div class="flex items-center gap-3">
            <!-- Live indicator -->
            <div class="flex items-center gap-2 px-3 py-1.5 rounded-full bg-white border border-gray-100 shadow-sm text-xs font-medium">
                {#if wsConnected}
    ...
    <div class="grid grid-cols-1 lg:grid-cols-5 gap-6">
        ...
        <div class="lg:col-span-2 flex flex-col gap-3">
            ...
            <div class="rounded-2xl p-6 text-center bg-white border border-gray-100 shadow-sm">
                ...
                <button
                    onclick={copyJoinUrl}
                    class="flex items-center gap-1.5 mx-auto text-xs transition-colors {copied ? 'text-green-500' : 'text-gray-400 hover:text-gray-600'}"
                >
                    {#if copied}
            ...
            <!-- Student counts -->
            <div class="flex items-stretch divide-x divide-gray-100 bg-white border border-gray-100 rounded-xl shadow-sm overflow-hidden">
                ...
                <div class="flex-1 px-4 py-3 text-center">
                    <div class="flex items-center justify-center gap-1.5 mb-0.5">
                        <div class="w-2 h-2 rounded-full bg-green-400 {connectedCount > 0 ? 'animate-pulse' : ''}"></div>
                    ...
                    {#if team.students.length > 0}
                        <div class="flex flex-wrap gap-1.5">
                            {#each team.students as student}
                                ...
                                <div class="flex items-center gap-1.5 px-2.5 py-1.5 rounded-full text-sm border"
                                     style={`background: ${team.color}10; border-color: ${team.color}25`}>
                                    <div class="w-2 h-2 rounded-full shrink-0 {isOnline ? 'bg-green-400 animate-pulse' : 'bg-gray-300'}"
                                         title={isOnline ? 'Connected' : 'Not yet on play page'}></div>
                        ...
                        {#each unassignedStudents as student}
                            ...
                            <div class="flex items-center justify-between bg-white px-4 py-2.5 rounded-xl border border-amber-100">
                                <div class="flex items-center gap-2">
                                    <div class="w-2 h-2 rounded-full {isOnline ? 'bg-green-400 animate-pulse' : 'bg-gray-300'}"></div>
