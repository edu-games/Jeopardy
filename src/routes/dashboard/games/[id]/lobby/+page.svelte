...
<div class="max-w-7xl mx-auto">
	...
	<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
		...
		<div class="bg-white rounded-lg shadow p-6">
			<h2 class="text-lg font-semibold text-gray-900 mb-4">Student Join</h2>

			<!-- QR Code -->
			<div class="mb-6">
				<img src={data.qrCode} alt="QR Code" class="w-full max-w-xs mx-auto border-4 border-gray-200 rounded-lg" />
			</div>
			...
			<!-- Join URL -->
			<div class="mb-4">
				<label class="block text-sm font-medium text-gray-700 mb-2">Join URL</label>
				<div class="flex items-center gap-2">
					<input
						type="text"
		...
		<div class="lg:col-span-2 space-y-4">
					...
					{#if team.students.length > 0}
						<div class="flex flex-wrap gap-2">
							{#each team.students as student}
								<div class="px-3 py-1 bg-gray-100 text-gray-800 rounded-full text-sm">
									{student.name}
								</div>
							{/each}
						</div>
					{:else}
						<p class="text-gray-500 text-sm italic">No students assigned yet</p>
					{/if}
				</div>
			{/each}
			...
			{#if data.game.students.length === 0}
				<div class="bg-white rounded-lg shadow p-8 text-center">
					...
					<p class="text-gray-600">Students will appear here as they join the game</p>
				</div>
			{/if}
		</div>
	</div>
