<script lang="ts">
	...
	let buzzerNotification = $state<{ studentName: string; teamName: string } | null>(null);
		...
		eventSource.onmessage = (event) => {
			try {
				...
				switch (eventData.type) {
					case 'connected':
						console.log('[SSE Instructor] Connected with client ID:', eventData.clientId);
						break;
			...
			<div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
				{#each teams as team}
					...
					>
						...
						<p class="text-xs text-blue-200">{team.students.length} students</p>
					</div>
				{/each}
			</div>
		</div>
			...
			<div class="grid grid-cols-6 gap-2 mb-2">
				{#each data.game.board.categories as category}
					<div class="bg-blue-900 p-4 rounded-lg text-center">
						<h3 class="text-yellow-400 font-bold text-sm uppercase tracking-wide">
							{category.name}
						</h3>
					</div>
				{/each}
...
{#if currentSlot}
	<div class="fixed inset-0 bg-black/80 flex items-center justify-center p-4 z-50">
		<div class="bg-blue-900 rounded-lg max-w-4xl w-full p-8 text-white shadow-2xl">
			<!-- Header -->
			<div class="flex justify-between items-start mb-6">
				<div>
					...
					<div class="text-3xl font-bold text-yellow-400">${currentSlot.points}</div>
					{#if currentSlot.isDailyDouble}
						<div class="mt-2 inline-block px-3 py-1 bg-yellow-500 text-blue-900 rounded-full text-sm font-bold">
							DAILY DOUBLE
						</div>
					{/if}
				</div>
			...
			<div class="mb-6 p-6 bg-blue-800 rounded-lg">
				<p class="text-sm text-blue-300 mb-2 uppercase tracking-wide">Clue</p>
				<p class="text-2xl">{currentSlot.question.answer}</p>
			</div>

			<!-- Question (Response) -->
			<div class="mb-8 p-6 bg-blue-700 rounded-lg">
				<p class="text-sm text-blue-300 mb-2 uppercase tracking-wide">Correct Response</p>
				<p class="text-2xl font-semibold text-yellow-400">{currentSlot.question.question}</p>
			</div>
				...
				<div class="grid grid-cols-2 md:grid-cols-3 gap-3">
					{#each teams as team}
						...
						>
							...
							<div class="text-sm text-blue-200">${team.score}</div>
						</button>
					{/each}
				</div>
			</div>
