<script lang="ts">
    ...
    function getStatusBadge(status: string) {
        switch (status) {
            case "LOBBY":       return "bg-amber-100 text-amber-700";
                    ...
                    <div class="flex items-center justify-between gap-4">
                        <div class="flex-1 min-w-0">
                            <div class="flex items-center gap-2.5 mb-1.5 flex-wrap">
                                ...
                                <span class={`px-2.5 py-0.5 rounded-full text-xs font-semibold ${getStatusBadge(game.status)}`}>
                                ...
                                <div class="flex flex-wrap gap-1.5 mt-3">
                                    {#each game.teams as team}
                                        <div class="flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium border"
                                            ...
                                            <div class="w-1.5 h-1.5 rounded-full shrink-0" style={`background: ${team.color}`}></div>
                                            {team.name}
...
{#if pendingDelete}
    <div class="fixed inset-0 z-50 flex items-center justify-center p-4"
        ...
        <div class="absolute inset-0 bg-black/20 backdrop-blur-sm"
             onclick={() => pendingDelete = null}></div>
            ...
            <div class="flex items-start gap-4 mb-5">
                ...
                <div>
                    ...
                    <p class="text-gray-500 text-sm mt-1">
                        <span class="font-semibold text-gray-700">{pendingDelete.name}</span> and all its data will be permanently deleted.
                    </p>
                </div>
            </div>
