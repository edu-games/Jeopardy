<script lang="ts">
	...
	let selectingSlot: { categoryIndex: number; slotIndex: number } | null = $state(null);
	...
	let filteredQuestions = $derived(() => {
		...
		const selectedQuestionIds = new Set(
			categories.flatMap((cat) =>
				cat.slots.map((slot) => slot.questionId).filter((id): id is string => id !== null)
			)
		);
		...
		if (questionSearch.trim()) {
			...
			filtered = filtered.filter(
				(q) => q.clue.toLowerCase().includes(term) || q.response.toLowerCase().includes(term)
			);
		}

		if (selectedTagFilter.length > 0) {
			filtered = filtered.filter((q) => q.tags.some((t) => selectedTagFilter.includes(t.tag.name)));
		}
	...
	function selectQuestion(questionId: string) {
		if (selectingSlot) {
			categories[selectingSlot.categoryIndex].slots[selectingSlot.slotIndex].questionId =
				questionId;
			selectingSlot = null;
		}
	}
	...
	let filledSlots = $derived(
		categories.reduce((sum, cat) => sum + cat.slots.filter((s) => s.questionId).length, 0)
	);
</script>
...
<div class="max-w-7xl mx-auto">
	<div class="mb-8">
		<div class="flex items-center gap-2 mb-2">
			<a href="/dashboard/boards" class="text-gray-600 hover:text-gray-900 transition-colors">
			...
			<h1 class="font-serif-display text-4xl text-gray-900">Create New Board</h1>
		</div>
		<p class="text-gray-600">Build a Tile Trivia board with 6 categories and 5 questions each</p>
	</div>
	...
	<div class="bg-white rounded-lg shadow p-6 mb-6">
		...
		<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
			<div>
				<label for="boardName" class="block text-sm font-medium text-gray-700 mb-2">
			...
			<div>
				<label for="boardDescription" class="block text-sm font-medium text-gray-700 mb-2">
	...
	<div class="bg-white rounded-lg shadow p-4 mb-6">
		<div class="flex justify-between text-sm text-gray-600 mb-2">
			<span>Progress</span>
			<span>{filledSlots}/30 slots filled ({Math.round((filledSlots / 30) * 100)}%)</span>
		</div>
	...
	<div class="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6 mb-6">
		{#each categories as category, categoryIndex}
			<div class="bg-white rounded-lg shadow p-6">
				...
				<!-- Slots -->
				<div class="space-y-2">
					{#each Array.from({ length: 5 }, (_, i) => category.slots[i]).filter((s) => s) as slot, slotIndex}
						{@const question = getQuestionById(slot.questionId)}
						<div
							class="border rounded-lg p-3 {slot.isWildCard
						...
						>
							...
							{#if question}
								...
								<button
									type="button"
									onclick={() => openQuestionSelector(categoryIndex, slotIndex)}
			...
			<div class="p-6 border-b">
				<div class="flex justify-between items-center mb-4">
					<h3 class="text-xl font-bold text-gray-900">
						Select Question for Category {selectingSlot.categoryIndex + 1}, Slot {selectingSlot.slotIndex +
							1}
					</h3>
					<button onclick={() => (selectingSlot = null)} class="text-gray-500 hover:text-gray-700">
