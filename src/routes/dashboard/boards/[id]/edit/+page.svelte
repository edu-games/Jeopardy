<script lang="ts">
	...
	// Board state - initialize from existing board
	let boardName = $state(data.board.name);
	let boardDescription = $state(data.board.description || '');
	...
	// Initialize from existing board data
	let categories = $state<Category[]>(
		data.board.categories.map((cat) => ({
			name: cat.name,
			...
			<div class="bg-white rounded-lg shadow p-6">
				<div class="mb-4">
					<label class="block text-sm font-medium text-gray-700 mb-2">
						Category {categoryIndex + 1} Name <span class="text-red-500">*</span>
					</label>
					<input
						type="text"
				...
				<div class="space-y-2">
					{#each category.slots as slot, slotIndex}
						...
						<div class="border rounded-lg p-3 {slot.isDailyDouble ? 'border-yellow-500 bg-yellow-50' : 'border-gray-200'}">
							<div class="flex items-center justify-between mb-2">
								...
								<div class="flex gap-1">
									<button
										type="button"
										onclick={() => (slot.isDailyDouble = !slot.isDailyDouble)}
							...
							{#if question}
								<div class="text-sm mb-2">
									<p class="text-gray-600 truncate">{question.answer}</p>
									<p class="text-blue-600 font-medium truncate">{question.question}</p>
								</div>
								<button
									type="button"
...
<!-- Question Selection Modal (same as new page) -->
{#if selectingSlot}
	<div
			...
			<div class="p-6 border-b">
				<div class="flex justify-between items-center mb-4">
					<h3 class="text-xl font-bold text-gray-900">
						Select Question for Category {selectingSlot.categoryIndex + 1}, Slot {selectingSlot.slotIndex + 1}
					</h3>
					<button
				...
				{:else}
					<div class="space-y-3">
						{#each filteredQuestions() as question}
							...
							>
								<p class="text-gray-600 mb-1">{question.answer}</p>
								<p class="text-blue-700 font-medium mb-2">{question.question}</p>
