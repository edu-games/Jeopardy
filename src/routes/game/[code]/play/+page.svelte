<script lang="ts">
	...
	let wildCardWager = $state<number | null>(null);
			...
			try {
				...
				switch (msg.type) {
					case 'question-revealed':
						...
						clueReady = !msg.currentSlot?.isWildCard;
						wildCardWager = null;
						if (gameState) {
	...
	const storageKey = `quizgame_session_${data.game.code}`;

	onMount(() => {
		localStorage.setItem(
			storageKey,
			JSON.stringify({ studentId: data.student.id, name: data.student.name })
		);
		connect();
	});
...
<div class="h-dvh flex flex-col overflow-hidden select-none font-sans bg-gray-50">
		...
		<div class="flex items-center justify-between">
			...
			<div class="text-right">
				{#if myTeam}
					...
					<p class="font-black text-lg leading-tight" style="color: {myTeam.color}">
	...
	<main class="flex-1 flex flex-col px-5 pb-4 pt-4 min-h-0 gap-3">
		{#if gameStatus === 'LOBBY'}
			<div class="flex-1 flex flex-col items-center justify-center text-center gap-6">
				...
				{#if myTeam}
					<div
						class="px-5 py-3 rounded-2xl border bg-white shadow-sm"
		...
		{:else if gameStatus === 'IN_PROGRESS'}
			{#if currentSlot && currentCategory}
				<!-- Category + points -->
				<div
					class="shrink-0 flex items-center justify-between px-4 py-2.5 rounded-2xl bg-white border border-gray-100 shadow-sm"
				>
				...
				{#if currentSlot.isWildCard && !clueReady}
					<!-- Wild Card splash — wager not yet submitted -->
					<div
				...
				{:else}
					<!-- Clue card -->
					<div
						class="flex-1 rounded-3xl flex items-center justify-center p-6 min-h-0 shadow-sm"
						style="background: linear-gradient(135deg, #1e3a8a, #1d4ed8)"
					>
						<p class="text-white text-2xl font-bold text-center leading-snug tracking-wide">
							{currentSlot.question.clue}
						</p>
					</div>
				{/if}
				...
				<div class="shrink-0 flex flex-col items-center gap-3 pb-1">
					{#if buzzerEnabled && !buzzerPressed}
						...
						>
							{#if pressingBuzzer}
								<svg
									class="animate-spin h-10 w-10 mx-auto text-white"
									fill="none"
									viewBox="0 0 24 24"
								>
									<circle
					...
					{:else if buzzerPressed}
						<div
							class="w-44 h-44 rounded-full flex flex-col items-center justify-center border-4 border-green-300/50"
						...
						>
							<svg
								class="w-12 h-12 text-white mb-1"
								fill="none"
								stroke="currentColor"
								viewBox="0 0 24 24"
							>
					...
					{:else}
						<div
							class="w-44 h-44 rounded-full flex flex-col items-center justify-center border-4 border-gray-200 bg-white shadow-sm"
						>
							<svg
						...
						<p class="text-gray-400 text-xs">
							{currentSlot.isWildCard ? 'Wild Card' : 'Wait for the buzzer'}
						</p>
					{/if}
				</div>
			{:else}
				...
				<div class="flex-1 flex flex-col items-center justify-center text-center gap-5">
					...
					{#if myTeam}
						<div
							class="px-4 py-2 rounded-xl text-sm font-bold bg-white border shadow-sm"
		...
		{:else if gameStatus === 'COMPLETED'}
			<div class="flex-1 flex flex-col items-center justify-center text-center gap-5">
				...
				{#if myTeam}
					<div
						class="px-5 py-4 rounded-2xl bg-white border shadow-sm text-center"
		...
		<div class="shrink-0 bg-white border-t border-gray-100 px-4 py-3">
			<div class="flex gap-2 overflow-x-auto" style="scrollbar-width: none">
				{#each sortedTeams as team, i (team.id)}
					<div
						class="shrink-0 flex items-center gap-1.5 px-3 py-1.5 rounded-full text-sm border transition-all
                                {team.id === myTeam?.id ? 'shadow-sm' : ''}"
						style="background: {team.color}10; border-color: {team.color}30; {team.id === myTeam?.id
							? `box-shadow: 0 0 0 2px ${team.color}40`
							: ''}"
					>
						...
						<span class="font-black" style="color: {team.color}"
	...
	{#if !wsConnected}
		<div
			class="absolute inset-x-0 bottom-0 flex items-center justify-center gap-2 py-2 bg-gray-900/80 backdrop-blur-sm"
		>
