		...
		try {
			...
			if (response.ok) {
				...
				localStorage.setItem(
					storageKey,
					JSON.stringify({ studentId: data.student.id, name: name.trim() })
				);
...
<div class="min-h-screen flex flex-col bg-gray-50 select-none">
	{#if !success}
		...
		<div class="flex-1 flex flex-col px-8 max-w-sm mx-auto w-full">
			{#if error}
				<div
					class="mb-6 flex items-center gap-2 bg-red-50 border border-red-100 rounded-2xl px-4 py-3"
				>
			...
			>
				<p class="text-gray-400 text-xs uppercase tracking-widest text-center mb-5">
	...
	{:else}
		...
		<div class="flex-1 flex flex-col items-center justify-center px-6 text-center">
			<div
				class="w-20 h-20 rounded-full flex items-center justify-center mb-6 bg-white border-2 shadow-sm"
			...
			{#if assignedTeam}
				<div
					class="px-8 py-4 rounded-2xl mb-6 bg-white border-2 shadow-sm"
				...
				>
					<div class="flex items-center justify-center gap-2 mb-0.5">
						...
						></div>
						<p class="text-lg font-black" style={`color: ${assignedTeam.color}`}>
							{assignedTeam.name}
						</p>
					</div>
