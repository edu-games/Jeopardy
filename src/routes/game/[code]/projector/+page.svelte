<script lang="ts">
	...
	const sortedTeams = $derived([...teams].sort((a, b) => (b.score ?? 0) - (a.score ?? 0)));
			...
			try {
				...
				switch (msg.type) {
					...
					case 'game-state-snapshot':
						...
						if (msg.currentSlot) {
							clueReady = !msg.currentSlot.isWildCard || !!msg.currentWager;
...
<div class="fixed inset-0 board-velvet flex flex-col">
	...
	<div class="shrink-0 px-6 py-4 border-b border-white/10 flex items-center justify-between gap-4">
		...
		<div
			class="bg-white/5 border border-white/10 px-3 py-1 rounded-full text-white/60 text-sm font-mono tracking-widest"
		>
			{data.game.code}
		</div>
	</div>
	...
	<div class="flex-1 flex flex-col px-6 py-4 gap-4 min-h-0">
		{#if currentSlot && currentCategory}
			{#if currentSlot.isWildCard && !clueReady}
				<!-- Wild Card splash — waiting for wager -->
			...
			{:else}
				<QuestionDisplay
					...
					isWildCard={currentSlot.isWildCard}
					clue={currentSlot.question.clue}
					wager={currentWager}
				/>
			{/if}
		{:else}
			<JeopardyBoard categories={data.game.board.categories} {answeredSlots} />
		{/if}
		...
		{:else if data.game.status === 'COMPLETED'}
			<div class="shrink-0 flex justify-center">
				<div
					class="bg-green-500/20 border border-green-500/40 text-green-400 rounded-full px-8 py-3 text-xl font-black"
				>
					Game Complete!
				</div>
			</div>
