<script lang="ts">
	...
	let myTeamRank = $derived(() => {
		...
		return data.game.teams.findIndex((t) => t.id === myTeam.id) + 1 || null;
	});
	let studentsPerTeam = $derived((teamId: string) => {
		return data.game.teams.find((t) => t.id === teamId)?.students.map((s) => s.name);
	});
</script>
...
<div class="min-h-screen board-velvet p-4">
	<div class="max-w-4xl mx-auto">
					...
					<div class="flex items-center justify-center gap-3 mb-3">
						...
						>
							...
							{:else if myTeamRank() === 3}
								🥉
							{:else}
								<span class="text-white font-bold">#{myTeamRank()}</span>
							{/if}
						</div>
		...
		<div class="bg-white/5 border border-white/10 rounded-2xl p-6 mb-6">
			<h3 class="text-xl font-bold text-white mb-4 text-center">Final Rankings</h3>
			<div class="space-y-3">
				{#each data.game.teams as team, index}
					{@const students = studentsPerTeam(team.id)}
					<div
						class="p-4 rounded-xl {team.id === myTeam?.id ? 'ring-2 ring-white/30' : ''}"
